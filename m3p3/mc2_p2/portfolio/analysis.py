...
def get_portfolio_value(prices, allocs, start_val=1):
    ...
    """
    # TODO: Your code here
    normalized_prices = prices / prices.ix[0, :]   # normalize the stock prices
    stock_value = normalized_prices * allocs * start_val    # calculate each stock value
    port_val = stock_value.sum(axis=1)    # sum the value of each stock
    return port_val
...
def get_portfolio_stats(port_val, daily_rf=0, samples_per_year=252):
    ...
    """
    # TODO: Your code here
    cum_ret = (port_val[-1] / port_val[0]) - 1    # cum_ret = final value / initial value - 1
    daily_ret = (port_val / port_val.shift(1)) - 1
    daily_ret[0] = 0
    avg_daily_ret = daily_ret[1:].mean()    # not include the first value
    std_daily_ret = daily_ret[1:].std()
    sharpe_ratio = (daily_ret[1:] - daily_rf).mean() / std_daily_ret * np.sqrt(samples_per_year)
...
def plot_normalized_data(df, title="Normalized prices", xlabel="Date", ylabel="Normalized price"):
    ...
    plot_data(nomalized_df, title=title, xlabel=xlabel, ylabel=ylabel)
